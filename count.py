...
def eye_tracking(ret, frame):
    ...
    return  threshold, gray_roi, whiteImg
...
while True:
    ...
    # 함수 호출
    threshold, gray_roi, whiteImg =eye_tracking(ret, frame)
    ...
    if k == ord('q'):
        ...
        while TIMER > 0:
            ...
            cv2.putText(whiteImg, str(TIMER), (500, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 0))
            cv2.imshow("Bin", threshold)
            cv2.imshow("Gray ROI", gray_roi)
            cv2.imshow("Eye tracking", whiteImg)
            cv2.waitKey(125)
            ...
            # if cenX < width and cenX > 0:
            #     if cenY < height and cenY > 0:
            #         count += 1
